...
import { CalendarPlus, CirclePlus, Heart, Loader2, Sparkles } from "lucide-react";
...
export default function Home() {
  ...
  return <div className="mx-auto max-w-5xl px-4 pb-8 pt-5 sm:px-7 lg:px-10 lg:pt-9">
    <header className="flex items-start justify-between gap-4"><div><p className="eyebrow">{copy.tagline}</p><h1 className="mt-1 font-display text-3xl sm:text-4xl">Your cycle, {displayName.split(" ")[0]}.</h1><p className="mt-2 max-w-xl text-sm leading-6 text-[#806A63]">Start with the dates that matter. Everything else can wait.</p></div><button onClick={() => setLocation("/profile")} className="grid h-11 w-11 place-items-center overflow-hidden rounded-full bg-[#E9D6D0] text-[#9E4B5A] transition hover:bg-[#DFBDB5]" aria-label="Open your profile">{data.profile.profilePhotoUrl ? <img src={data.profile.profilePhotoUrl} alt="Your Redtent profile" className="h-full w-full object-cover" /> : displayName.slice(0, 1).toUpperCase()}</button></header>
    <div className="mt-7"><CycleCalendarCard month={visibleMonth} marks={calendar.data} isLoading={calendar.isLoading} status={periodStatus} onPrevious={() => setVisibleMonth(new Date(visibleMonth.getFullYear(), visibleMonth.getMonth() - 1, 1))} onNext={() => setVisibleMonth(new Date(visibleMonth.getFullYear(), visibleMonth.getMonth() + 1, 1))} onAddPeriod={() => setLocation("/cycle")} onOpenCycle={() => setLocation("/cycle")} /></div>
