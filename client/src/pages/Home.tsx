...
export default function Home() {
  ...
  return <div className="mx-auto max-w-5xl px-4 pb-8 pt-4 sm:px-7 sm:pt-5 lg:px-10 lg:pt-9">
    ...
    <section className="mt-6"><div className="flex items-end justify-between gap-4"><div><p className="eyebrow">Log today</p><h2 className="mt-1 font-display text-2xl">A few small ways to stay connected.</h2></div><button onClick={() => setLocation("/wellness")} className="text-sm font-semibold text-[#800020]">Open check-in</button></div><div className="mt-4 grid gap-3 sm:grid-cols-4"><QuickAction icon={Heart} title="Mood" detail={data.todayWellness?.mood ? "Logged today" : "Name how you feel"} onClick={() => setLocation("/wellness")} /><QuickAction icon={CirclePlus} title="Energy" detail={data.todayWellness?.energy ? "Logged today" : "A small signal"} onClick={() => setLocation("/wellness")} /><QuickAction icon={Sparkles} title="Food Lens" detail="See food in context" onClick={() => setLocation("/food")} /><QuickAction icon={CalendarHeart} title="Care" detail="Find a careful next step" onClick={() => setLocation("/care")} /></div></section>
    <section className="mt-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-[#E8D6DB] bg-[#FFF8F5] px-5 py-4"><p className="max-w-2xl text-sm leading-6 text-[#755E57]">Your forecast is personal and can shift. It is a planning tool, not a diagnosis or a promise. If a pattern feels worrying, you can review it privately and consider a qualified healthcare professional.</p><button onClick={() => setLocation("/ask")} className="inline-flex items-center gap-2 rounded-xl bg-[#F8ECEF] px-4 py-2.5 text-sm font-semibold text-[#800020] transition hover:bg-[#F0D5DB]"><Sparkles className="h-4 w-4" /> Ask Redtent</button></section>
  </div>;
}
