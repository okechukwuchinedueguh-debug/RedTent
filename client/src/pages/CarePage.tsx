...
export default function CarePage() {
  ...
  return <div className="mx-auto max-w-6xl px-4 pb-8 pt-5 sm:px-7 lg:px-10 lg:pt-9">
    <header className="max-w-3xl"><p className="eyebrow">Redtent Care</p><h1 className="mt-1 font-display text-3xl leading-[1.02] sm:text-5xl">Care, when a next step feels useful.</h1><p className="mt-3 text-sm leading-6 text-[#75565B] sm:text-base">Redtent connects understanding to action. Start with your private health story, then choose a care path that fits your location, needs, and comfort.</p></header>

    <section className="redtent-care-hero mt-7 p-5 sm:p-8"><div className="max-w-2xl"><p className="redtent-care-hero__eyebrow text-xs font-bold uppercase tracking-[0.18em]">A careful connection layer</p><h2 className="mt-3 font-display text-3xl leading-tight sm:text-4xl">Your data stays private. Your next step stays yours.</h2><p className="mt-4 text-sm leading-6 text-[#F5E6DF] sm:text-base">Redtent Care is being built with locally verified partners. Until those connections are live, we do not display unverified provider profiles, availability, prices, or booking options.</p><div className="mt-6 flex flex-wrap gap-3"><button type="button" onClick={showCarePreparation} className="redtent-care-hero__button px-4 py-3">Explore care categories</button><button type="button" onClick={() => setLocation("/patterns")} className="rounded-xl border border-white/25 px-4 py-3 text-sm font-bold text-white hover:bg-white/10">Review your patterns</button></div></div></section>
