...
import { BookmarkPlus, Loader2, ShieldCheck, Sparkles } from "lucide-react";
...
export default function AskRedtentPage() {
  ...
  const savedConversationId = useMemo(() => {
    const value = new URLSearchParams(location.split("?")[1] || "").get("conversation");
  ...
  const saveConversation = trpc.ask.conversations.create.useMutation({
    onSuccess: async () => { await utils.ask.conversations.list.invalidate(); toast.success("Conversation saved in Your Space"); },
