...
export default function AskRedtentPage() {
  ...
  return <div className="mx-auto max-w-5xl px-4 pb-8 pt-5 sm:px-7 lg:px-10 lg:pt-9"><header className="max-w-3xl"><p className="eyebrow">Ask Redtent</p><h1 className="mt-1 font-display text-3xl sm:text-4xl">Ask about the moments that matter to you.</h1><p className="mt-3 text-sm leading-6 text-[#806A63]">Ask Redtent is a thoughtful layer across your cycle, Food Lens, wellness check-ins, and Your Space. It shares general information and practical options, not medical diagnosis or treatment.</p></header><section className="mt-7 grid gap-5 lg:grid-cols-[0.7fr_1.3fr]"><aside className="rose-card h-fit p-5"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-xl bg-[#F7E7E3] text-[#B45263]"><ShieldCheck className="h-5 w-5" /></div><div><p className="eyebrow">You choose the context</p><h2 className="font-display text-xl">Keep it as private as you need.</h2></div></div><div className="mt-5 space-y-3"><ContextToggle label="Recent wellness check-ins" detail="Mood, energy, sleep, and symptoms" checked={includeWellness} onChange={setIncludeWellness} /><ContextToggle label="Food Lens snapshots" detail="Visible foods from recent saved snapshots" checked={includeFood} onChange={setIncludeFood} /><ContextToggle label="Your Space" detail="Up to three recent journal entries" checked={includeJournal} onChange={setIncludeJournal} /></div><p className="mt-5 text-xs leading-5 text-[#927B74]">Your choices apply to the next question. Redtent does not claim information you have not chosen to share.</p></aside><section className="overflow-hidden rounded-[1.7rem] border border-[#E7D8D3] bg-[#FFFDFB] shadow-[0_14px_36px_rgba(103,65,60,0.08)]"><div className="border-b border-[#F0E4DF] bg-[#FCF4F0] p-5"><div className="flex items-center gap-3"><Sparkles className="h-5 w-5 text-[#B45263]" /><div><p className="eyebrow">Redtent AI</p><p className="font-display text-xl">Clear context for everyday choices.</p></div></div></div><AIChatBox messages={messages} onSendMessage={send} isLoading={ask.isPending} height="580px" placeholder="Ask Redtent anything about your recent context..." emptyStateMessage="What would you like to understand today?" suggestedPrompts={prompts} className="rounded-none border-0 shadow-none" /></section></section></div>;
}
