...
export default function FoodPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 pb-8 pt-5 sm:px-7 lg:px-10 lg:pt-9">
      <header><p className="eyebrow">AI food insights for your cycle</p><h1 className="mt-1 font-display text-3xl sm:text-4xl">See your meal in the bigger picture.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#806A63]">Take a food photo to uncover a cycle-aware nutrition snapshot with macro estimates, micronutrient highlights, and gentle next-step ideas. This is context for your wellbeing, never a measure of your worth.</p></header>
      <section className="mt-7 grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
        <div className="overflow-hidden rounded-[1.7rem] bg-[#512F37] p-6 text-[#FFF9F7] shadow-[0_18px_50px_rgba(77,36,47,0.2)] sm:p-8">
          ...
          <h2 className="mt-5 font-display text-3xl">Turn one meal into a useful insight.</h2>
          <p className="mt-3 max-w-lg text-sm leading-6 text-[#F0DEDA]">We look at what is visible, then return macro estimates, micronutrient highlights, and flexible ideas for your current cycle phase.</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            <button onClick={launchCamera} className="flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-3.5 text-sm font-bold text-[#713647] transition hover:bg-[#FAEDEC]"><Camera className="h-4 w-4" /> Take a meal photo</button>
            <button onClick={() => galleryInput.current?.click()} className="flex items-center justify-center gap-2 rounded-xl border border-white/35 bg-white/10 px-4 py-3.5 text-sm font-bold text-white transition hover:bg-white/15"><ImagePlus className="h-4 w-4" /> Choose a meal photo</button>
          </div>
        ...
        <aside className="rose-card p-6"><ShieldCheck className="h-7 w-7 text-[#778F72]" /><h2 className="mt-4 font-display text-2xl">Helpful context, never judgment.</h2><p className="mt-3 text-sm leading-6 text-[#806A63]">Photos cannot confirm portions, ingredients, cooking methods, or personal health needs. Redtent makes uncertainty clear and never treats an image estimate as fact.</p><ul className="mt-5 space-y-2 text-sm text-[#6E5650]"><li className="flex gap-2"><span className="text-[#B65464]">•</span> Approximate macro ranges</li><li className="flex gap-2"><span className="text-[#B65464]">•</span> Visible-food micronutrient highlights</li><li className="flex gap-2"><span className="text-[#B65464]">•</span> Flexible ideas for your cycle phase</li></ul></aside>
      </section>
      {selected && <section className="rose-card mt-7 overflow-hidden p-4 sm:p-6"><div className="grid gap-5 sm:grid-cols-[220px_1fr]"><img src={selected.dataUrl} className="h-52 w-full rounded-2xl object-cover sm:h-44" alt="Selected meal ready for analysis" /><div className="flex flex-col items-start"><p className="eyebrow">Your next insight is ready</p><h2 className="mt-1 font-display text-2xl">This meal is yours to understand, not judge.</h2><p className="mt-3 text-sm leading-6 text-[#806A63]">Your photo and insight stay in your Redtent account. Review it, learn from it, or remove it whenever you choose.</p><div className="mt-5 flex flex-wrap gap-3"><button onClick={() => analyse.mutate(selected)} disabled={analyse.isPending} className="inline-flex items-center gap-2 rounded-xl bg-[#A84D5F] px-5 py-3 text-sm font-semibold text-white disabled:opacity-60">{analyse.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <ScanLine className="h-4 w-4" />}{analyse.isPending ? "Building your meal insight..." : "Get my meal insight"}</button><button onClick={() => setSelected(null)} disabled={analyse.isPending} className="rounded-xl bg-[#F4E5E0] px-4 py-3 text-sm font-semibold text-[#8E4B57]">Choose a different photo</button></div></div></div></section>}
      {currentAnalysis && <AnalysisResult result={currentAnalysis} onDismiss={() => setCurrentAnalysis(null)} />}
      <section className="mt-8"><div className="flex items-end justify-between"><div><p className="eyebrow">Your meal insight library</p><h2 className="mt-1 font-display text-2xl">Return to the meals that taught you something.</h2></div><span className="text-xs text-[#8D756C]">{foods.data?.length || 0} saved</span></div>{foods.data?.length ? <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">{foods.data.map(entry => <FoodHistoryCard key={entry.id} entry={entry} onDelete={id => remove.mutate({ id })} />)}</div> : <div className="rose-card mt-4 grid min-h-48 place-items-center p-6 text-center"><Upload className="h-7 w-7 text-[#BE6C76]" /><p className="mt-3 max-w-md text-sm leading-6 text-[#806A63]">Your meal insight library begins when you want more context. There is no pressure to log every meal.</p></div>}</section>
