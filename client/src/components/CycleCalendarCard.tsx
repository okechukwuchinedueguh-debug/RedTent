...
export default function CycleCalendarCard({ month, marks, isLoading, status, onPrevious, onNext, onAddPeriod, onOpenCycle }: CycleCalendarCardProps) {
  ...
  return <section className="rose-card overflow-hidden p-4 sm:p-6">
    ...
    <div className={`mt-5 rounded-2xl border p-4 ${statusStyles(status.kind)}`}><p className="text-xs font-bold uppercase tracking-[0.16em]">Next period estimate</p><p className="mt-1 font-display text-xl sm:text-2xl">{status.title}</p><p className="mt-1 max-w-2xl text-sm leading-6 opacity-90">{status.detail}</p></div>
...
function CalendarDay({ mark, isExpectedPastDate }: { mark: CalendarMark; isExpectedPastDate: boolean }) {
  ...
  return <div className={`relative flex aspect-square min-h-9 flex-col items-center justify-center rounded-lg text-[11px] sm:min-h-13 sm:rounded-xl sm:text-xs ${color} ${isToday ? "ring-2 ring-[#513039] ring-offset-2" : ""} ${isExpectedPastDate ? "ring-2 ring-[#B85262] ring-offset-1" : ""}`} title={isExpectedPastDate ? "Past estimated period date" : mark.isPredictedPeriod ? "Estimated period day" : mark.isLoggedPeriod ? "Logged period day" : "Cycle day"}><span className="font-bold">{mark.date.getDate()}</span>{isExpectedPastDate ? <span className="mt-0.5 hidden text-[8px] font-bold sm:block">EST.</span> : null}</div>;
}
