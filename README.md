...
| `drizzle/schema.ts` | User-scoped tables and indexed relations for cycle logs, wellness entries, journals, food entries, preparation routines, moment reflections, and notification preferences. |
...
Partner companion access is currently disabled. Existing partner-related records are retained privately for possible future reactivation, but no partner route, shared-device selection, or companion view is exposed in the active product.
...
Your Patterns is intentionally conservative. It only reflects explicitly logged, user-scoped records and never creates an observation until enough related entries exist. The pre-period checklist and moment reflections are optional general-wellness tools, not requirements or clinical records. Email delivery remains selected for future Resend configuration but is deliberately inactive until a verified sender and secure credentials are provided. Browser preference controls therefore do not send messages in this release.
